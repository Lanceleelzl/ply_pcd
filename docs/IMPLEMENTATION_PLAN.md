...
- 已经用户确认设计：左侧集中模型、粗配准、ICP 与结果，结果详情在第 4 步下方按指标及两类矩阵纵向展开；视口左上角悬浮辅助工具条，剖切、原点平面或坐标查询面板从视口右侧悬浮覆盖并让导航立方体水平左移；坐标查询开关和显示位置切换保持当前相机姿态。
- 复用现有面板、命令与状态，保持元素标识及配准算法不变；仅调整布局与展示。保持相机姿态，验证工具面板互斥、结果展开、主流程入口及不同桌面宽度。
