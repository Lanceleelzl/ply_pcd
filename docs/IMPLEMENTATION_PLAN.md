...
- Worker 继续求原始文件坐标矩阵。服务输出业务矩阵 `P_b × T_file_a_to_b × inverse(P_a)`，并保存文件矩阵、预变换及业务矩阵。
- 坐标查询输入输出均为业务坐标；原始文件清理后历史业务矩阵仍可用于换算。

### 2026-09-07 续接检查与待确认方案
...
- 待确认的修正方案：单独维护文件局部坐标下的刚体粗配准矩阵；业务预变换仅参与业务显示、坐标查询和最终矩阵组合。通过分层实体或完整仿射显示链保留非等比缩放与旋转产生的剪切，不能将组合矩阵直接拆为单个实体的 TRS。
...
- 本方案属于跨模块显示状态调整，待用户确认后实施；现阶段不得宣称业务预变换端到端完成。
