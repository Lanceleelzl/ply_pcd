...
- 配准视图统一使用固定模型业务坐标系，以固定模型业务原点作为显示偏移；分层保留业务 TRS 与文件局部刚体矩阵，禁止将含剪切的组合矩阵分解为单层 TRS。查询「原始位置」显示各模型自己的业务预变换。粗配准状态独立使用双精度刚体矩阵，Gizmo 通过独立代理换算显示位置和旋转，禁止从含缩放的显示矩阵反求 ICP 初值。
